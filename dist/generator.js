...
export function generate(schema, opts) {
    ...
    for (const table of ordered) {
        ...
        for (const c of table.columns) {
            if (c.unique || c.primaryKey)
                uniqueTrackers.set(c.name, new Set());
        }
        const rows = [];
...
function rawValue(col, index, generated) {
    switch (col.type) {
        case "fk": {
            if (!col.references)
                return null;
...
export function toCSV(result) {
    ...
    for (const t of result.tables) {
        ...
        const lines = t.rows.map((row) => t.columns.map((c) => csvCell(row[c])).join(","));
        blocks.push(`# ${t.name}\n${header}\n${lines.join("\n")}`);
    }
    return blocks.join("\n\n");
}
