import { SchemaDef, TableDef, ColumnDef, ColumnType } from "./types.js";

// Accepts either a JSON schema object/string or raw SQL DDL and returns a SchemaDef.
export function parseSchema(input: string): SchemaDef {
...
export function parseDDL(ddl: string): SchemaDef {
  ...
  if (tables.length === 0) {
    throw new Error(
      "Could not parse any CREATE TABLE statements. Pass valid SQL DDL or a JSON schema."
    );
  }
  return { tables };
}
...
function mapSqlType(colName: string, sqlType: string, tableName: string): ColumnType {
  ...
  if (name === "url" || name.endsWith("_url")) return "url";
  if (name === "name" || name.endsWith("_name")) {
    if (name.includes("first")) return "firstName";
    if (name.includes("last")) return "lastName";
    if (name.includes("user")) return "username";
    if (name.includes("company") || name.includes("org")) return "company";
    // Explicit person signals in the column name.
    if (
      /(full|contact|customer|author|owner|employee|person|client|manager|recipient|sender|guest|member)/.test(
        name
      )
    )
      return "fullName";
    ...
    if (
      /(user|customer|person|people|member|contact|employee|author|client|profile|account|student|patient|guest|attendee|staff)/.test(
        table
      )
    )
      return "fullName";
    ...
    if (/(product|item|sku|catalog|good|inventory|listing)/.test(table))
      return "productName";
    if (/(city|cities|town|place)/.test(table)) return "city";
    if (/(countr)/.test(table)) return "country";
